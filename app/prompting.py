# TODO(rewrite-debug): Remove this file once the rewrite-debug work is done.
# It is retained here for reference and potential future use.
if __name__ == "__main__":
    print("This file is deprecated and retained for reference only.")
# app/prompting.py
"""
Prompting utilities for HR Ask Local RAG system.

Includes:
- build_system_prompt(): CPO persona & answer rules for the answering model.
- build_user_prompt(query): wraps the (possibly rewritten) user query.
- Lightweight multi-turn rewriter:
...
REWRITE_SYSTEM_PROMPT = """
You are a cautious query rewriter for an HR analytics RAG system.

Your single job:
- If the user's new message is a follow-up that depends on the last exchange,
  rewrite it into a fully standalone query that preserves the user's intent.
- If the new message is already standalone, return it unchanged.

STRICT RULES:
- Use ONLY the immediately previous exchange (last user question + last assistant answer) as context.
- Be minimal. Do not add new constraints, data, or assumptions.
- Keep original entities (teams, dates, metrics) unless the user explicitly changes them.
- If the new message is a refinement (filter, scope, attribute, timeframe), merge it into the previous intent.
- Output ONLY the final query text. No explanations, no JSON, no markup.
"""
...
REWRITE_FEW_SHOTS: List[Dict[str, str]] = [
    ...
    {"role": "assistant", "content": "how does compensation work relative to the performance review cycle?"},
    ...
    {"role": "assistant", "content": "show open roles (including contractors) by org and seniority"},
]

# ----------------------- rewriter helpers ------------------------------------

_REFERENTIAL_SUBSTRINGS = [
    "what about", "how about", "just ", "only", "instead", "also", "same",
    "that too", "include", "exclude", "filter", "narrow", "broaden",
    "except", "w/o", "w/out", "remove", "vs ", "versus ",
]
