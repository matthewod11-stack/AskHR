...
if os.getenv("SHOW_REWRITE_DEBUG", "false").lower() not in ("1", "true", "yes"):
...
SCENARIOS = {
    "Attrition": [
        ...
        {
            "name": "Exclude interns / last 90d",
            "last_user": "show attrition by department",
            "last_assistant": "Attrition is 12% overall. Sales 18%, Eng 10%, Ops 8%.",
            "new_user": "last 90 days, exclude interns",
        },
    ],
    "Headcount": [
        {
        ...
        {
            "name": "By location (incl contractors)",
            "last_user": "show headcount trends for 2023",
            "last_assistant": "Grew from 60 to 90 employees.",
            "new_user": "by location SF, include contractors",
        },
    ],
    "DEI": [
        {
        ...
        {
            "name": "Senior IC vs last quarter",
            "last_user": "show DEI metrics for Q2 2024",
            "last_assistant": "Improved leadership +3%.",
            "new_user": "women in senior IC (L5+) vs last quarter",
        },
    ],
    "Recruiting": [
        {
            "name": "Include contractors",
        ...
        {
            "name": "SR roles H2 only",
            "last_user": "pipeline health for software engineer roles",
            "last_assistant": "Pass-through rate 28%, offer-accept 71%.",
            "new_user": "last 6 months, senior roles only",
        },
    ],
    "Perf & Comp": [
        {
            "name": "Compensation linkage",
        ...
        {
            "name": "Engineers only",
            "last_user": "calibration guidelines",
            "last_assistant": "Use distribution bands; edge cases via VP approval.",
            "new_user": "for engineers only",
        },
    ],
    "Policies": [
        {
            "name": "Short PIP",
            "last_user": "PIP policy and template",
            "last_assistant": "Standard 60-day PIP with weekly checkpoints.",
            "new_user": "shorter 30-day version?",
        },
        {
            "name": "CA hourly PTO",
            "last_user": "PTO policy overview",
            "last_assistant": "Unlimited exempt; hourly accrue 1h/30h worked.",
            "new_user": "California hourly only",
        },
    ],
    "Edge Cases": [
        {
            "name": "Standalone (no rewrite)",
            "last_user": "",
            "last_assistant": "",
            "new_user": "show headcount by department and location for 2022–2024 with quarterly granularity",
        },
        {
            "name": "SQL guard (no rewrite)",
            "last_user": "show attrition by department",
            "last_assistant": "Sales 18%, Eng 10%, Ops 8%.",
            "new_user": "SELECT dept, COUNT(*) FROM employees WHERE terminated_at >= '2024-01-01';",
        },
    ],
}
...
selected_idx = st.sidebar.selectbox(
    "Scenario", range(len(scenarios)), format_func=lambda i: scenario_names[i]
)
...
if st.button("Run A/B"):
    ...
    raw_data = (
        raw_resp.json() if raw_resp.status_code == 200 else {"answer": "Error", "citations": []}
    )
    ...
    rewritten_data = (
        rewritten_resp.json()
        if rewritten_resp.status_code == 200
        else {"answer": "Error", "citations": []}
    )
    ...
    for i, scenario in enumerate(scenarios):
        with st.expander(f"{scenario['name']}"):
            ...
            raw_resp = requests.post(
                f"{API_URL}/v1/ask", json={"query": scenario["new_user"], "k": k}
            )
            raw_data = (
                raw_resp.json()
            ...
            rewritten_query = (
                debug.get("rewritten_query", scenario["new_user"])
                if debug
                else scenario["new_user"]
            )
            rewritten_resp = requests.post(
                f"{API_URL}/v1/ask", json={"query": rewritten_query, "k": k}
            )
            rewritten_data = (
                rewritten_resp.json()
                if rewritten_resp.status_code == 200
            ...
            st.markdown("**Raw Citations:**")
            for c in raw_data.get("citations", []):
                st.write(c)
            st.markdown(f"**Rewritten Answer:** {rewritten_data.get('answer','')}")
            st.markdown("**Rewritten Citations:**")
            for c in rewritten_data.get("citations", []):
                st.write(c)
